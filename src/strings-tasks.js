...
function formatTime(minutes, seconds) {
  ...
  const result = `${finalMinutes}:${finalSeconds}`;
  return result;
}
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (!symbols.includes(str[i])) {
      correctedStr += str[i].toLowerCase();
    }
  }
  let reversedStr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (!symbols.includes(str[i])) {
      reversedStr = str[i].toLowerCase() + reversedStr;
    }
  }
  return reversedStr === correctedStr;
}
...
function findLongestWord(sentence) {
  ...
  const arrOfLength = arrOfWords.map((item) => item.length);
  const arrSorted = arrOfLength.sort((a, b) => b - a);
  const longestLength = arrSorted[0];
  const arrOfLongestWords = arrOfWords.filter(
    (item) => item.length === longestLength
  );

  return arrOfLongestWords[0];
}
...
function reverseWords(str) {
  ...
  for (let j = 0; j < arrOfWords.length; j += 1) {
    let reversedWord = '';
    for (let i = 0; i < arrOfWords[j].length; i += 1) {
      reversedWord = arrOfWords[j][i] + reversedWord;
    }
...
function encodeToRot13(str) {
  const firstPartChars =
    'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz ?!';

  const secondPartChars =
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm ?!';
  ...
  for (let i = 0; i <= str.length; i += 1) {
    if (firstPartChars.includes(str[i])) {
      charIndex = firstPartChars.lastIndexOf(str[i]);
      result += secondPartChars[charIndex];
    }
  }
